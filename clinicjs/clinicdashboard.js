...
//Total Number
var librarydatabase = firebase.database().ref('Clinic');
       
    //Patient Record
    librarydatabase.child("Patients Record").once("value").then(function(snapshot)
    {
        document.getElementById("totalclinicpatient").innerHTML = snapshot.numChildren();
    });
    //Student Record
    librarydatabase.child("Students Record").once("value").then(function(snapshot)
    {
        document.getElementById('totalclinicstudent').innerHTML = snapshot.numChildren();
    });
